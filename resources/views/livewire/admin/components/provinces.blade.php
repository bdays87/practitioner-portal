<div>
    ...
    <div class="p-5 rounded-sm border bg-base-200 border-gray-200 hover:bg-blue-100 transition-colors duration-200 cursor-pointer" wire:click="$set('modal', true)">          
        ...
        <h2 class="text-lg font-bold text-center">Provinces</h2>        
    </div>
    <x-modal title="Provinces" wire:model="modal" box-class="max-w-3xl">
        <x-input label="Search" wire:model.live="search" />
        <x-table :headers="$headers" :rows="$provinces">
            @scope('actions', $province)
            <div class="flex items-center space-x-2">
                @can('configurations.modify')
                <x-button icon="o-pencil" class="btn-sm btn-info btn-outline" 
                    wire:click="edit({{ $province->id }})" spinner />
                <x-button icon="o-trash" class="btn-sm btn-outline btn-error" 
                    wire:click="delete({{ $province->id }})" wire:confirm="Are you sure?" spinner />
                @endcan
            </div>
            @endscope
            <x-slot:empty>
                <x-alert class="alert-error" title="No provinces found." />
            </x-slot:empty>
        </x-table>
    ...
    <x-modal title="{{ $id ? 'Edit province' : 'New province' }}" wire:model="modifymodal">

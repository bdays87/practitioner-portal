<div>
    ...
    <x-card title="Renewal Fees" separator class="mt-5 border-2 border-gray-200">
        <x-slot:menu>
        ...
        <x-table :headers="$headers" :rows="$records">
            ...
            @endscope
            <x-slot:empty>
                <x-alert class="alert-error" title="No renewal fees found." />
            </x-slot:empty>
        </x-table>
    </x-card>
    @else
    <x-alert class="alert-error mt-3" title="You do not have permission to view renewal fees." />
    @endcan

    <x-modal wire:model="modal" title="{{ $id ? 'Edit Renewal Fee' : 'New Renewal Fee' }}">
        <x-form wire:submit="{{ $id ? 'update' : 'save' }}">
            <div class="grid grid-cols-2 gap-2">
                ...
                <x-input label="General Ledger" wire:model="generalledger" placeholder="Enter general ledger code" />

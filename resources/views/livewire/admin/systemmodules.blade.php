<div>
    <x-breadcrumbs :items="$breadcrumbs"    class="bg-base-300 p-3 rounded-box mt-2" />
    <x-card title="System Modules" separator class="mt-5 border-2 border-gray-200">
        <x-slot:menu>
            <x-button label="New" responsive icon="o-plus" class="btn-outline" @click="$wire.modal = true" />
        </x-slot:menu>

        <x-table :headers="$headers" :rows="$modules">
            @scope('actions', $module)
            <div class="flex items-center space-x-2">
                <x-button icon="o-pencil" class="btn-sm btn-info btn-outline" 
                    wire:click="edit({{ $module->id }})" spinner />
                <x-button icon="o-trash" class="btn-sm btn-outline btn-error" 
                    wire:click="delete({{ $module->id }})" confirm="Are you sure?" spinner />
                <x-button icon="o-link" class="btn-sm btn-outline btn-info" 
                    link="{{ route('submodules.index', $module->id) }}" spinner />
            </div>
            @endscope
            <x-slot:empty>
                <x-alert class="alert-error" title="No modules found." />
            </x-slot:empty>
        </x-table>
    </x-card>
    <x-modal wire:model="modal"  title="{{ $id ? 'Edit System Module' : 'New System Module' }}">
        <x-form wire:submit="{{ $id ? 'update' : 'save' }}">
            <div class="grid grid-cols-2 gap-2">
            <x-input label="Name" wire:model="name" />
            <x-select label="Account Type" wire:model="accounttype_id" placeholder="Select Account Type" :options="$accounttypes" option-label="name" option-value="id" />
            <x-input label="Icon" wire:model="icon" />
            <x-input label="Default Permission" wire:model="default_permission" />
            </div>
         

            <x-slot:actions>
                <x-button label="Cancel" @click="$wire.modal = false" />
                <x-button label="{{ $id ? 'Update' : 'Save' }}" type="submit" class="btn-primary" spinner="save" />
            </x-slot:actions>
        </x-form>
    </x-modal>
</div>
